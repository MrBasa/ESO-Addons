...
LootLog = {
	...
	GOLD_ICON = " |t16:16:EsoUI/Art/currency/currency_gold.dds|t",
...
-- Lookup table for gold transaction reasons
LootLog.MoneyReasonToString = {
	[0]  = "Loot",
	[1]  = "Vendor",
	[2]  = "Mail",
	[3]  = "Trade",
	...
	[28] = "Mount Upgrade",
	[29] = "Repair",
	[31] = "Guild Trader Purchase",
	[33] = "Guild Trader Listing Fee",
	[42] = "Bank Deposit",
	[43] = "Bank Withdrawl",
	[51] = "Guild Bank Deposit",
	[52] = "Guild Bank Withdrawl",
	[59] = "Pickpocket",
	[60] = "Launder Fee",
	[62] = "Stolen",
	[63] = "Fence",
}
-- TODO: Adjust recipient columns, make sure recipients do not wrap
...
local function OnAddOnLoaded( eventCode, addonName )
	...
	if (not LootLogCustomHistory) then LootLogCustomHistory = { } end
	if (not LootLogCustomHistory[server]) then LootLogCustomHistory[server] = { } end
	LootLog.history = LootLogCustomHistory[server]
	
	-- Initialize gold history
	if (not LootLogCustomGoldHistory) then LootLogCustomGoldHistory = { } end
	if (not LootLogCustomGoldHistory[server]) then LootLogCustomGoldHistory[server] = { } end
	LootLog.goldhistory = LootLogCustomGoldHistory[server]
	...
	LCCC.RunAfterInitialLoadscreen(function( )
		...
		EVENT_MANAGER:RegisterForEvent(LootLog.name, EVENT_MONEY_UPDATE, LootLog.OnMoneyUpdate)
...
function LootLog.OnMoneyUpdate(eventCode, currentMoney, previousMoney, reason, subReason)
	-- Add a guard to ensure the money values are valid numbers before proceeding
	if type(currentMoney) ~= "number" or type(previousMoney) ~= "number" then
		d(string.format("LootLog DEBUG - Invalid money amounts -> reason: %s, subReason: %s, currentMoney: %s, previousMoney: %s", tostring(reason), tostring(subReason), currentMoney, previousMoney))
	end
	
	local delta = currentMoney - previousMoney
	if delta ~= 0 then
		-- If the reason isn't in our known list, print a debug message
		if not LootLog.MoneyReasonToString[reason] then
			d(string.format("LootLog DEBUG - Unknown reason -> reason: %s, subReason: %s, delta: %d", tostring(reason), tostring(subReason), delta))
		end

		-- Pass the numeric reason code directly
		LootLog.LogGold(delta, reason)
	end
end
...
function LootLog.PackGold(timestamp, amount, reason, userId, charName )
	return string.format("%d;%d;%d;%s;%s", timestamp, amount, reason, userId, charName)
end

function LootLog.UnpackGold( entry )
	if (type(entry) == "string") then
		entry = { zo_strsplit(";", entry) }
		entry[1] = tonumber(entry[1]) -- timestamp
		entry[2] = tonumber(entry[2]) -- amount
		entry[3] = tonumber(entry[3]) -- reason
	end
	return entry
end

function LootLog.LogGold(amount, reason)
	local timestamp = GetTimeStamp()
	local key = zo_floor(timestamp / 3600)

	if (not LootLog.goldhistory[key]) then LootLog.goldhistory[key] = { } end

	table.insert(LootLog.goldhistory[key], LootLog.PackGold(timestamp, amount, reason, LootLog.self.userId, LootLog.self.name))
...
function LootLog.ExpireOldData( )
	...
	for key, _ in pairs(LootLog.history) do
		if (type(key) == "number" and currentKey - key > LootLog.vars.historyHours) then
			LootLog.history[key] = nil
			expired = true
		end
	end
	
	for key, _ in pairs(LootLog.goldhistory) do
		if (currentKey - key > LootLog.vars.historyHours) then
			LootLog.goldhistory[key] = nil
			expired = true
		end
	end
...
function LootLog.InitializePriceSourceComboBox(comboBox, callback)
	local choices = {
		{ text = "NPC Vendor", value = LibPrice.NPC },
	}

	if LibPrice.CanTTCPrice and LibPrice.CanTTCPrice() then
		table.insert(choices, { text = "Tamriel Trade Centre", value = LibPrice.TTC })
	end

	if LibPrice.CanMMPrice and LibPrice.CanMMPrice() then
		table.insert(choices, { text = "Master Merchant", value = LibPrice.MM })
	end

	local currentSource = LootLog.vars.priceSource
	local isSourceValid = false
	for _, choice in ipairs(choices) do
		if choice.value == currentSource then
			isSourceValid = true
			break
		end
	end
	if not isSourceValid then
		LootLog.vars.priceSource = LibPrice.NPC
	end

	comboBox:ClearItems()

	for _, choiceData in ipairs(choices) do
		local entry = comboBox:CreateItemEntry(choiceData.text, callback)
		entry.value = choiceData.value
		comboBox:AddItem(entry)
		if choiceData.value == LootLog.vars.priceSource then
			comboBox:SetSelectedItem(choiceData.text)
		end
	end
end

function LootLog.FormatCurrencyOld(value, decimals)
    if type(value) ~= "number" then return value end
    decimals = (decimals == nil) and 2 or decimals
    
    -- Use the built-in game function to handle rounding, decimals, and comma separators
	LootLog.Msg(string.format("Value: %s - Decimals: %s"))
    return ZO_LocalizeDecimalNumber(tonumber(value), decimals, true)
end

function LootLog.FormatCurrency(value, decimals)
    if type(value) ~= "number" then return value end
    
    decimals = (decimals == nil) and 2 or decimals
    
    -- Handle the special case for zero to show a dash for prices but "0" for gold
    if value == 0 then
        return decimals == 0 and "0" or ZO_LocalizeDecimalNumber(0)
    end

    -- Use the game's built-in function to round to the nearest value (e.g., 0.01 for 2 decimals)
    local precision = 1 / (10^decimals)
    local roundedValue = zo_roundToNearest(value, precision)

    -- Use the game's built-in localization function to add commas and handle regional formats
    return ZO_LocalizeDecimalNumber(roundedValue, decimals, true)
end
