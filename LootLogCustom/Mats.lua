...
function LootLogMats:Setup( )
	...
	local sortKeys = {
		...
		["unitPrice"] = { isNumeric = true, tiebreaker = "item" },
		["totalPrice"] = { isNumeric = true, tiebreaker = "item" },
		["inStock"] = { isNumeric = true, tiebreaker = "item" },
	}
	...
	local function OnPriceSourceChanged(comboBox, choiceText, choice)
		...
		self.scratch = { processed = { }, counts = { }, time = 0 }
		self:RefreshData()
	end
...
function LootLogMats:BuildMasterList( )
	...
	for itemLink, count in pairs(self.scratch.counts) do
		...
		local foundPrice = LibPrice.ItemLinkToPriceGold(itemLink, LootLog.vars.priceSource)
		local unitPrice = foundPrice or 0
		local totalPrice = foundPrice and (foundPrice * count) or 0
		...
		table.insert(self.masterList, {
			...
			count = count,
			inStock = stackCountBackpack + stackCountBank + stackCountCraftBag,
			unitPrice = unitPrice,
...
function LootLogMats:FilterScrollList( )
	...
	self.frame:GetNamedChild("Counter"):SetText(string.format("%s: %d%s", "Items", #scrollData, grandTotalText))
end
...
function LootLogMats:SetupItemRow( control, data )
	...
	cell = control:GetNamedChild("InStock")
	cell.normalColor = ZO_DEFAULT_TEXT
	cell:SetText(data.inStock)
