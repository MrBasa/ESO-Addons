...
local TAB_NAME = "LootLogCustomHistory"
...
function LootLogList:Setup( )
	...
	local sortKeys = {
		...
		["unitPrice"] = { isNumeric = true, tiebreaker = "time", tieBreakerSortOrder = ZO_SORT_ORDER_DOWN },
		["totalPrice"] = { isNumeric = true, tiebreaker = "time", tieBreakerSortOrder = ZO_SORT_ORDER_DOWN },
	}
	...
	--self.frame:GetNamedChild("PriceSourceLabel"):SetText(GetString(SI_LOOTLOG_PRICE_SOURCE))
	self.priceSourceDrop = ZO_ComboBox_ObjectFromContainer(self.frame:GetNamedChild("PriceSourceDrop"))
	local function OnPriceSourceChanged(comboBox, choiceText, choice)
		LootLog.vars.priceSource = choice.value
		-- Force a full refresh by clearing the cached data
		self.masterList = { }
		self.scratch = { processed = { } }
		self:RefreshData()
	end
	LootLog.InitializePriceSourceComboBox(self.priceSourceDrop, OnPriceSourceChanged)
	...
	for key, group in pairs(LootLog.history) do
		...
		for i = self.scratch.processed[key] + 1, #group do
			...
			if type(entry[2]) ~= "number" then			
				local foundPrice = LibPrice.ItemLinkToPriceGold(entry[2], LootLog.vars.priceSource)
				local unitPrice = foundPrice or 0
				local totalPrice = foundPrice and (foundPrice * entry[3]) or 0
				...
				LootLog.Msg(string.format("Bad Item: %s - %s", entry[2], LootLog.vars.priceSource))
			end
		end
	end
end
...
function LootLogList:FilterScrollList( )
			...
			if (not LootLog.vars.historyShowUncollected or LootLogMulti.ShouldFlagAsUncollected(data.itemLink, LootLogMulti.CONTEXT_HISTORY_FILTER, nil, data.personal)) then
				if (searchInput == "" or self.search:IsMatch(searchInput, data)) then
					...
					if data.totalPrice then
						grandTotal = grandTotal + data.totalPrice
					end
				end
			end
		end
	...
	local grandTotalText = ""
	if grandTotal > 0 then
		LootLog.Msg(string.format("Grand Total: %s", grandTotal))
		grandTotalText = string.format(" — %s: %s%s", "Total", LootLog.FormatCurrency(grandTotal), LootLog.GOLD_ICON)
	end

	if (#scrollData ~= #self.masterList) then
		self.frame:GetNamedChild("Counter"):SetText(string.format("%s: %d / %d%s", "Items", #scrollData, #self.masterList, grandTotalText))
	else
		self.frame:GetNamedChild("Counter"):SetText(string.format("%s: %d%s", "Items", #self.masterList, grandTotalText))
	end
end
...
function LootLogList:SetupItemRow( control, data )
	...
	cell = control:GetNamedChild("UnitPrice")
	cell.normalColor = ZO_DEFAULT_TEXT
	local unitPriceText = (data.unitPrice and data.unitPrice > 0) and (LootLog.FormatCurrency(data.unitPrice) .. LootLog.GOLD_ICON) or "-"
	cell:SetText(unitPriceText)

	cell = control:GetNamedChild("TotalPrice")
	cell.normalColor = ZO_DEFAULT_TEXT
	local totalPriceText = (data.totalPrice and data.totalPrice > 0) and (LootLog.FormatCurrency(data.totalPrice) .. LootLog.GOLD_ICON) or "-"
	cell:SetText(totalPriceText)
